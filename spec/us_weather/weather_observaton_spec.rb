...
describe USWeather::WeatherObservation do
  describe "current" do
    it "returns the current observation" do
      ...
      observation = USWeather::WeatherObservation.current("KCCR")
    ...
    it "returns an existing observation if it already exists" do
      ...
      existing_observation = USWeather::WeatherObservation.create!(icao_id: "KCCR", observed_at: time, description: "Clear")
      observation = USWeather::WeatherObservation.current("KCCR")
    ...
    it "returns nil if the NOAA API returns a 404" do
      ...
      expect(USWeather::WeatherObservation.current("KCCR")).to be_nil
    end
  end
  ...
  describe "current!" do
    ...
    it "raises an error if the current observation cannot be found" do
      stub_request(:get, "https://api.weather.gov/stations/KCCR/observations/latest").to_return(status: 404)
      expect { USWeather::WeatherObservation.current!("KCCR") }.to raise_error(ArgumentError)
    end
  end
